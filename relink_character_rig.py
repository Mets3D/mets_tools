...
def reload_proxied_rig(context, rig, blendfile=None):
	...
	blendfile:	If supplied, use this filepath to reload the collection from a different file than where it was previously linked from.
	"""
...
class ReProxy_Rigs(bpy.types.Operator):
	...
	def execute(self, context):
		...
		for rig in context.selected_objects:
			print("Working on: " + rig.name)
			reload_proxied_rig(context, rig)
		return { 'FINISHED' }
